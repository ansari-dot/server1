    ...
    static async register(req, res) {
        try {
            ...
            if (address && (!address.country || address.country.trim() === '')) {
                console.log('Address validation failed - country required:', address);
                return res.status(400).json({
                    success: false,
                    message: 'Country is required when providing address'
                });
            }
    ...
    static async updateProfile(req, res) {
        try {
            ...
            const user = await User.findByIdAndUpdate(
                req.user.userId, { name, phone, addresses }, { new: true, runValidators: true }
            ).select('-password');
